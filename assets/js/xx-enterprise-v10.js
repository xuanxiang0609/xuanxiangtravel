 ...
 * 全站 JS 核心｜防呆、轉換追蹤、表單保護、圖片保護、Header/Footer 串接保護
 */
(function () {
  "use strict";

  if (window.__XX_ENTERPRISE_V10_BOOTED__) return;
  ...
  function track(eventName, params = {}) {
    ...
    const payload = {
      ...
      site_version: cfg.VERSION || "Enterprise v10",
      ...params
    };
  ...
  function initClickTracking() {
    if (window.__XX_V10_CLICK_TRACKING__) return;
  ...
  function initImageGuard(root = document) {
    $$("img", root).forEach((img) => {
      if (img.dataset.xxV10ImageReady === "1") return;
      img.dataset.xxV10ImageReady = "1";
  ...
  function initFormGuard() {
    $$("form").forEach((form) => {
      if (form.dataset.xxV10FormReady === "1") return;
      form.dataset.xxV10FormReady = "1";
  ...
  function initQuoteQuickEvents() {
    ...
    if (quoteBtn && quoteBtn.dataset.xxV10QuoteReady !== "1") {
      quoteBtn.dataset.xxV10QuoteReady = "1";
  ...
  function initBookingFunnel() {
    ...
    if (!bookingForm || bookingForm.dataset.xxV10BookingReady === "1") return;

    bookingForm.dataset.xxV10BookingReady = "1";
  ...
  function observeDom() {
    if (window.__XX_V10_OBSERVER__) return;
  ...
  function init() {
    ...
    document.dispatchEvent(new CustomEvent("xx:enterprise-v10-ready", {
      detail: {
        version: "Enterprise Black Gold v10.0",
        ready: true
      }
    }));
  }
