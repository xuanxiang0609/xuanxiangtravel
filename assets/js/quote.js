/*
 * 玹翔旅遊 Ultimate Final v12｜Booking Mall Quote Engine
 * 功能：輸入框防遮罩、地址提示、Google Maps 距離、機場固定價優先、夜間加價、偏遠地區加價、v12 車型命名、預約商城帶入、LINE 一鍵送出、GA4 事件。
 */
(function () {
  "use strict";

  const CONFIG = window.XUANXIANG_CONFIG || window.XX_CONFIG || window.XX || {};
  const QUOTE_VERSION = "Ultimate Final v12 Quote Engine";
  const BOOKING_MALL_VERSION = String(CONFIG.BOOKING_MALL_VERSION || CONFIG.BOOKING_VERSION || "v12");
  ...
  const VEHICLE_NAME = {
    ...
    van: "豪華九人座",
    vito: "VITO 商務九座",
    alphard: "Alphard / Lexus LM",
    vip: "Alphard / Lexus LM",
    sprinter: "明星保母車｜Sprinter"
  };

  const VEHICLE_TO_BOOKING = {
    sedan: "舒適五人座",
    van: "豪華9人座",
    vito: "VITO",
    alphard: "阿法",
  ...
  function renderLoading() {
    ...
    if (!box) return;
    box.innerHTML = `
      <div class="card quote-result-card" style="padding:22px;border-radius:22px;">
  ...
  function renderError(message, hint = "") {
    ...
    if (!box) return;
    box.innerHTML = `
      <div class="card quote-result-card" style="padding:22px;border-radius:22px;border-color:rgba(255,80,80,.35);">
  ...
  function buildQuote(data) {
    ...
    const bookingVehicle = VEHICLE_TO_BOOKING[vehicle] || VEHICLE_TO_BOOKING.sedan;
    ...
    return {
      ...
      bookingVehicle: VEHICLE_TO_BOOKING[vehicle] || VEHICLE_TO_BOOKING.sedan,
  ...
  function getLineUrl(q) {
    ...
    const bookingUrl = getBookingUrl(q);
    const text = [
      "您好，我想詢問玹翔旅遊即時報價：",
  ...
  function getBookingUrl(q) {
    const url = new URL("booking.html", location.href);
    url.searchParams.set("from", "quote");
    url.searchParams.set("quoteVersion", QUOTE_VERSION);
    url.searchParams.set("service", "長途接送");
    url.searchParams.set("serviceDetail", "單程");
    url.searchParams.set("date", getValue("quoteDate"));
    url.searchParams.set("time", getValue("quoteTime"));
    url.searchParams.set("pickup", getValue("quotePickup"));
    url.searchParams.set("dropoff", getValue("quoteDropoff"));
    url.searchParams.set("luggage", getValue("quoteLuggage"));
    url.searchParams.set("vehicle", q.bookingVehicle || VEHICLE_TO_BOOKING.sedan);
    url.searchParams.set("estimatedMin", String(q.minPrice));
    url.searchParams.set("estimatedMax", String(q.maxPrice));
    url.searchParams.set("pricingMode", q.pricingMode);
    return url.href;
  }
  ...
  function renderResult(q) {
    ...
    box.innerHTML = `
      <div class="card quote-result-card">
        <span class="eyebrow">${q.airportFixed ? "⭐ 機場固定價優先" : "📍 Google Maps 即時估價"}</span>
        <h2 class="quote-result-price">${money(q.minPrice)} ～ ${money(q.maxPrice)}</h2>
        <p class="quote-result-meta">
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:18px;">
          ...
          <a class="btn" href="${escapeHtml(bookingUrl)}" data-quote-go-booking>帶入資料前往預約商城</a>
        </div>
      </div>
    `;
    ...
    box.querySelector("[data-quote-go-booking]")?.addEventListener("click", function () {
      track("bookingStart", {
        source: "quote_engine",
        quote_version: QUOTE_VERSION,
        booking_mall_version: BOOKING_MALL_VERSION,
        vehicle: q.vehicle,
        estimated_min: q.minPrice,
        estimated_max: q.maxPrice
      });
    }, { once: true });
  }
  ...
  function calculateQuote() {
    ...
    if (window.XXAnalytics?.trackEvent) {
      window.XXAnalytics.trackEvent("quote_input_check", {
        quote_version: QUOTE_VERSION,
        booking_mall_version: BOOKING_MALL_VERSION,
        has_pickup: Boolean(pickup),
  ...
  document.dispatchEvent(new CustomEvent("xx:quote-ready", {
    detail: {
      version: QUOTE_VERSION,
      bookingMallVersion: BOOKING_MALL_VERSION,
      mapsKeyReady: isRealMapsKey(getMapsKey())
    }
  }));
})();