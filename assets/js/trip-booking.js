/ *
 * 玹翔旅遊 Ultimate Final v8.1 Enterprise｜旅遊活動共用送單正式上線防呆版
 * 功能：日期防呆、電話驗證、數量金額保護、API 逾時、重複送單防護、草稿暫存、GA4 事件、LINE fallback。
 * /
(function () {
  "use strict";
  ...
  const API = String(cfg.APPS_SCRIPT_URL || "").trim();

  const SETTINGS = {
    sourcePrefix: "trip-booking",
  ...
  async function submit(options = {}) {
    ...
    try {
      ...
      alertMessage(`預約資料已送出！訂單編號：${orderId}。客服會再確認車款、報價與成行狀況。`, "success");
